...
merge_by_substring <- function(data, key_df, search_col, key_col, value_cols,
  ...
  # Combine results and merge with original data using data.table for speed
  match_results <- data.table::rbindlist(match_list[seq_len(k)])
  rownames(match_results) <- NULL

  # Use data.table merge for better performance
  data_dt <- data.table::as.data.table(data)
  match_results_dt <- data.table::as.data.table(match_results)
  data.table::setkeyv(match_results_dt, search_col)

  result <- data.table::merge.data.table(data_dt, match_results_dt,
    by = search_col, all.x = TRUE,
